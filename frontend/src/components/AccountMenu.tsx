...
import { DATAOS_FQDN } from "../lib/auth";
...
// Falls back to a demo examiner identity when no DataOS session is present.
...
export function AccountMenu({ collapsed = false }: { collapsed?: boolean }) {
  ...
  const email = user?.email ?? "onboarding · executive group";
    ...
    <>
      ...
      <div className="px-2 py-2">
        ...
        <button
          ...
          onClick={() => {
            setOpen(false);
            window.location.href = `${DATAOS_FQDN}/`;
          }}
        >
